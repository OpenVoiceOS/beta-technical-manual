...
| `date_format` | `"MDY"` | Date format for display and parsing. |
| `confirm_listening` | `true` | Play a beep when the system starts listening. |
| `open_data.intent_urls` | `[]` | Opt-in list of open-data servers to help improve the intent pipeline by sharing anonymized match data. A dashboard for contributed data is at [opendata.tigregotico.pt](https://opendata.tigregotico.pt). |
...
| `open_data.user_agent` | (none) | Optional `User-Agent` string sent with open-data uploads. |
...
| `postag.module` | `"ovos-classifiers-postag-plugin"` | Ships from `ovos-classifiers`. **Warning: archived.** The shipped default still works, but the repo is no longer maintained. |
| `g2p.module` | `""` |  |
