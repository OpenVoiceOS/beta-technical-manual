...
!!! warning "raspOVOS-specific commands"
    This guide applies to raspOVOS and may assume some raspOVOS-exclusive utilities are available. If you are not using raspOVOS, [some command line utilities](https://github.com/OpenVoiceOS/raspOVOS/tree/dev/overlays/base_ovos/usr/local/bin) will not be available.
...
??? example "Fix"
    ...
    - **Speaker Demographics:** Precise models, including `precise-lite`, are often trained with datasets predominantly
      ...
      such as children's or women's voices. This is a common issue also seen in [Speech-to-Text](stt-plugins.md) ([STT](stt-plugins.md)) models.
    ...
    Picking a good wake word improves the accuracy and responsiveness of your system. Here are some tips for
    choosing a wake word that works well in various environments:
...
??? example "Fix"
    ...
    !!! tip
        Set `"save_utterances": true` in your [listener config](https://github.com/OpenVoiceOS/ovos-config/blob/V0.0.13a19/ovos_config/mycroft.conf). Recordings will be saved to `~/.local/share/mycroft/listener/utterances`.
    ...
    By default OVOS applies [VAD](vad-plugins.md) ([Voice Activity Detection](vad-plugins.md)) to crop silence from the audio sent to STT, this helps in
    ...
    !!! tip
        Set `"remove_silence": false` in your [listener config](https://github.com/OpenVoiceOS/ovos-config/blob/V0.0.13a19/ovos_config/mycroft.conf). This will send the full audio recording to STT.
    ...
    !!! tip
        Set `"instant_listen": false` in your [listener config](https://github.com/OpenVoiceOS/ovos-config/blob/V0.0.13a19/ovos_config/mycroft.conf). This will drop the listen sound audio from the STT audio buffer. You will need to wait for the listen sound to finish before speaking your command in this case.
