...
!!! abstract "In a nutshell"
    The rest of this manual documents individual building blocks (settings, scheduling, playback, a GUI) one at a time. This page instead shows them **combined**, the way a real skill uses them: a complete, working file for each common job a skill needs to do. If you already know what `schedule_event` or `@ocp_search` does and just want to see it used correctly next to everything else it needs, start here. Newer skill authors are welcome too: the table below is ordered as a difficulty ladder — the plain `OVOSSkill` recipes near the top are approachable straight after [Your First Skill](first-skill.md), while the conversational-game, OCP, and MQTT recipes further down assume more.
