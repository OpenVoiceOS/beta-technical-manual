...
1. [Install with ovos-installer](ovos-installer.md): the guided path for most hardware, including an existing Raspberry Pi OS setup.
2. [RaspOVOS image](install-raspovos.md): pick this instead if you want a flash-and-boot SD card image for a spare Raspberry Pi.
3. [Make it yours](personalize.md): change the wake word, voice, and language once the assistant is running.
4. [Fun stuff to try](showcase.md): a tour of things to build once the basics work.
5. [What can I say? (Skills)](skill-examples.md): a catalog of the default skills you can talk to right away.
