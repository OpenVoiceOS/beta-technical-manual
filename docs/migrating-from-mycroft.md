...
| `settingsmeta.json` / `settingsmeta.yaml` | Still read, no backend to sync to | The legacy settings-UI descriptor still works for tools that read it, but nothing uploads it anywhere. See [Settings Meta](skill-settings-meta.md) and [Skill Settings](skill-settings.md). |
