...
*   `updated_session`: lets a plugin mutate session state as part of matching. It must be the session the round is already running on (OVOS-PIPELINE-1 §4.2). The orchestrator compares its `session_id` against the round's, and a mismatch is logged as an error and ignored, with the round continuing on its own session. Read the session with `SessionManager.get(message)` and return that same object mutated, rather than constructing a fresh one. Even on a matching `session_id`, the [deployment-owned fields](session.md#deployment-owned-fields) are re-imposed from the value the orchestrator held before the plugin ran, so a plugin cannot use this to relax a blacklist, redirect the transformer chain, or move `site_id`.
