...
Persona definitions are different: the full reference file bind-mounts a host directory
(`${OVOS_PERSONA_FOLDER}`) rather than using a named volume, so that data persists only as
the host path you manage yourself.
...
default (config key `gui_websocket.base_port`, see [Bus Service](bus-service.md)). Both the
bus and the GUI socket ship bound to `127.0.0.1`. Widening `gui_websocket.host` to `0.0.0.0`
(required for remote display clients) combined with host networking puts the unauthenticated
GUI socket on the whole LAN, not just the device — keep the default unless a remote display
client needs it, and check upgraded configs that may carry an old `0.0.0.0` value.
