...
- The per-session `intent_context` (`session.intent_context`) decays: entries carry an
  `expires_at` computed from the `context.timeout` config (minutes, default 2), or a
  `turns_remaining` budget, and a re-set refreshes the expiry. This includes entries
  written through `set_context` (see [Conversational Context](context.md)). This is what
  "expires," not the session itself. The
