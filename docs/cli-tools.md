...
| `ovos-config set -k <key> -v <value>` | Write a value. Omit `-v` to be prompted (useful when the key is ambiguous). See the [known bug](config.md#usage-guide) — this currently writes to the wrong file. |
