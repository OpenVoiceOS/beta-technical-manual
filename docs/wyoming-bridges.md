...
!!! note "Synthesis is whole-utterance"
    The bridge handles `Describe` and `Synthesize` only. It synthesizes the full text in one
    call and chunks the resulting WAV back over the wire. Sentence-by-sentence text streaming
    is not part of this bridge, so a client that needs it has to split the text itself and send
    one `Synthesize` per sentence.
