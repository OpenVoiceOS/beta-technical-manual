...
!!! warning "What you should see in the log when a whitelist blocks converse"
    ...
    shows up as a candidate. No `{skill_id}.converse.request` / `skill.converse.response`
    pair for it appears at all, and its `converse()` method is never invoked. If a skill you expect to converse is
