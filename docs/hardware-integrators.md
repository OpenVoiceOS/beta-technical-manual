...
| Wake word / STT / TTS / VAD plugins | **Any Linux SBC / PC** | CPU (or GPU) inference; heavier models just need more RAM/CPU. For concrete sizing, use the [Raspberry Pi tiers](install-raspovos.md#step-1-prepare-your-hardware) as a baseline: a Pi 3-class board handles wake word + cloud speech only, Pi 4-class (4 GB) runs local ONNX STT/TTS comfortably, Pi 5-class adds headroom for Whisper-class models |
