...
| ovos-tts-plugin-phoonnx (built into [phoonnx](https://github.com/TigreGotico/phoonnx)) | no license file | the `phoonnx` repository ships no `LICENSE` file. See the [TTS plugin table](tts-plugins-reference.md) for the corresponding model-card caveat |
