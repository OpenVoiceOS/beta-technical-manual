...
| [ovos-stt-plugin-citrinet](stt-plugins-reference.md#ovos-stt-plugin-citrinet) | OpenVoiceOS STT plugin. Warning: deprecated by its own README in favor of [onnx-asr](stt-plugins-reference.md#ovos-stt-plugin-onnx-asr), which runs the same Citrinet models and more. | Apache-2.0 (model: see model card) | Stable |
