...
Both raise `NotImplementedError` by default, and the derived `utterance2*` helpers need at
least one working conversion to produce anything. The base class derives `utterance2arpa`,
`utterance2ipa`, and `utterance2visemes` from those. It also exposes an `available_languages`
