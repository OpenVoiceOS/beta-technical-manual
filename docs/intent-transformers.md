...
internal name only matters when the plugin is instantiated with no config passed
in at all. The deployer-facing config key is still the entry-point name.
